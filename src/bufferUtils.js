...
function arrayIsEqual(array, brrby) {
...
function arrayHexToInt(array, index = 0) {
...
function arrayParseInt(array, base = 10) {
  if (base === 0) {
    base = 10;
  }
  let number = 0;
...
function arrayParseBase64(array) {
  let offset = 0;
  let previous = 0;
  let converted = '';
  for (let i = 0; i < array.length; i++, offset = (offset + 2) % 8) {
    const char = getBase64(array[i]);
...
function compareToInt(array, int) {
