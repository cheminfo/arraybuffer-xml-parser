// Vendored copy of getTraversableGenerator for the A/B benchmark: jumps to the next `<` with a clamped native scan
// Both variants live here so the comparison stays runnable at any commit.
// Only the scan loop differs; every helper is imported unchanged from src.
import { XMLNode } from '../../src/XMLNode.ts';
import { arrayIndexOf } from '../../src/bufferUtils/arrayIndexOf.ts';
import { arrayTrim } from '../../src/bufferUtils/arrayTrim.ts';
import { closingIndexForOpeningTag } from '../../src/traversable/closingIndexForOpeningTag.ts';
import type { RealStreamParseOptions } from '../../src/traversable/defaultOptions.ts';
import { findClosingIndex } from '../../src/traversable/findClosingIndex.ts';
import { parseAttributesString } from '../../src/traversable/parseAttributesString.ts';
import { removeNamespaceIfNeeded } from '../../src/traversable/utils/removeNamespaceIfNeeded.ts';
import { decoder } from '../../src/traversable/utils/utf8Decoder.ts';

export async function* getTraversableGeneratorIndexOf(
